...
interface DiningRoomSectionProps {
  className?: string;
}

export default function DiningRoomSection({ className = '' }: DiningRoomSectionProps) {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Wipe transition to light background
      scrollTl.fromTo(
        wipeRef.current,
        { y: '100%' },
  ...
  return (
    <section
      ref={sectionRef}
      id="dining-room-section"
      className={`pinned-section bg-[#0B0B0D] ${className}`}
    >
      {/* Wipe panel for transition to light section */}
      <div
        ref={wipeRef}
        className="absolute inset-0 bg-[#F4F1EC] z-50 pointer-events-none"
        style={{ transform: 'translateY(100%)' }}
      />

      {/* Oversized outline text "DINING" */}
      <div
        ref={outlineTextRef}
      ...
      >
        Tables, seating, and lighting—composed for gathering.
      </p>
